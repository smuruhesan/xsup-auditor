...
- Two retrospective audit workers + one independent knowledge-artifact worker.
- Supports direct KCS generation from either an XSUP ID or an SFDC case number without running the retrospective field-review prompt.
...
- If the triggering field values cannot be established from original ticket evidence, Retrospective Eligibility is UNDETERMINED.`
  ...
  function artifactValidationItems(artifact) {
    ...
    if (!normalized || /^(none|none identified|not applicable|n\/a|no additional validation items(?: identified)?)[.!]?$/i.test(normalized)) {
      return [];
    }
    ...
    const lines = String(section)
      ...
      .map(line => cleanText(
        line
          .replace(/^\s*(?:[-*+]|\d+[.)])\s*/, "")
          .replace(reviewMarkerPattern(), "")
      ))
      .filter(Boolean)
      .filter(line => !/^(none|none identified|not applicable|n\/a)$/i.test(line));
  ...
  function qualityValidationItems(job) {
    const validation = cleanText(job?.knowledgeQualityValidationItems || "");
    if (!validation || /^(none|none identified|n\/a|not applicable)$/i.test(validation)) return [];
    return validation
      .split(/(?:\r?\n|\s*;\s*|\s+·\s+)/)
      .map(cleanText)
      .filter(Boolean);
  ...
  function knowledgeReviewCount(job) {
    ...
    const articleItems = artifactValidationItems(job.knowledgeAnswer || "");
    const qualityItems = qualityValidationItems(job);
    const blocking = knowledgeBlockingReviewItems(job);

    // Inline material markers are the preferred representation. Article/quality
    // validation lists frequently repeat those same concerns, so only use the
    // lists as the primary count when the article has no material inline markers.
    let count = stats.material.length;
    if (!count) {
      count = Math.max(articleItems.length, qualityItems.length);
    }

    // A quality-execution/finalization blocker is distinct from content-level
    // validation. Count it once unless an UNSUPPORTED blocker is already visible.
    if (blocking.length && !stats.blockers.length) {
  ...
  function knowledgeMarkdownToHtml(markdown) {
    ...
    // Render the mandatory At a Glance section as a compact visual summary
    // rather than another large article heading.
    let atAGlanceHtml = "";
    text = text.replace(
      /(?:^|\n)##\s+At a Glance\s*\n([\s\S]*?)(?=\n##\s+|$)/i,
      (_, body) => {
        const summaryBody = safeMarkdownToHtml(String(body || "").trim());
        atAGlanceHtml = `
          <aside class="xa-at-a-glance" aria-label="At a glance">
            <div class="xa-at-a-glance-title">ℹ At a glance</div>
            <div class="xa-at-a-glance-body">${summaryBody}</div>
          </aside>
        `;
        return "\n@@XA_AT_A_GLANCE_BLOCK@@\n";
      }
    );
    ...
    if (atAGlanceHtml) {
      html = html
        .replace(/<p>\s*@@XA_AT_A_GLANCE_BLOCK@@\s*<\/p>/i, atAGlanceHtml)
        .replace(/@@XA_AT_A_GLANCE_BLOCK@@/g, atAGlanceHtml);
    }
  ...
  function knowledgeIssueWhy(issue) {
    const s = cleanText(issue || "");
    if (!s) return "The quality review could not establish this item safely.";

    if (/Source References/i.test(s)) {
      return "The underlying source support is incomplete or not direct enough to treat the affected guidance as publication-ready.";
    }
    if (/missing required section/i.test(s)) {
      return "A required part of the reusable article is missing, so the draft may be incomplete for another engineer.";
    }
    if (/unsupported review marker/i.test(s)) {
      return "At least one material statement is still explicitly marked unsupported and must be sourced, rewritten, or removed.";
    }
    if (/raw internal provenance|internal provenance/i.test(s)) {
      return "Internal reasoning/provenance syntax must not appear in a user-facing knowledge artifact.";
    }
    if (/raw source\/provenance markup|verification-warning|<ref/i.test(s)) {
      return "Internal source markup must be converted into clean human-readable references and any verification requirement must remain visible.";
    }
    if (/internal reuse metadata/i.test(s)) {
      return "Internal tool metadata must never be exposed in the article. The generator should remove it automatically before review.";
    }
    if (/placeholder|token/i.test(s)) {
      return "Unresolved machine/editorial placeholder text can make the article misleading or unusable.";
    }
    if (/code fence/i.test(s)) {
      return "Malformed code formatting can change commands or make procedural instructions unsafe to copy.";
    }
    if (/artifact targets/i.test(s)) {
      return "The generated draft appears to reference a different source target and must be checked before reuse.";
    }
    if (/At a Glance/i.test(s)) {
      return "The summary should let an engineer understand the topic, key finding, and purpose before reading the full article.";
    }
    if (/quality finalization|QUALITY_REVIEW_ERROR|Case Chat task rejected|Service error \(422\)/i.test(s)) {
      return "The draft exists, but the independent quality-review service did not complete successfully. Content-level validation must remain visible until the quality review completes.";
    }
    return "The independent quality/deterministic review found this item could not yet be treated as publication-ready.";
  }

  function knowledgeBlockingReviewItems(job) {
    if (!job) return [];
    const readiness = normalizeDecision(job.validatedArtifactReadiness || job.artifactReadiness);
    if (readiness !== "NOT READY") return [];

    const raw = [
      ...(Array.isArray(job.knowledgeQualityIssues) ? job.knowledgeQualityIssues : []),
      ...(Array.isArray(job.knowledgeDraftReviewIssues) ? job.knowledgeDraftReviewIssues : [])
    ].map(cleanText).filter(Boolean);

    if (!raw.length && job.knowledgeQualitySummary) raw.push(cleanText(job.knowledgeQualitySummary));
    if (!raw.length && job.knowledgeError) raw.push(cleanText(job.knowledgeError));
    if (!raw.length) raw.push("This draft requires human review before publication.");
  ...
  function knowledgeBlockingReviewHtml(job) {
    const items = knowledgeBlockingReviewItems(job);
    if (!items.length) return "";

    return `
      <div class="xa-publication-blocker" role="alert">
        <div class="xa-publication-blocker-title">
          <span class="xa-semantic-chip xa-semantic-red">✕ REVIEW REQUIRED</span>
          <strong>Draft generated successfully — not ready for publication</strong>
        </div>
        <p>The generator completed the draft instead of discarding it. Review the items below before publishing or treating the content as final.</p>
        <ul>
          ${items.map(issue => `
            <li>
              <strong>What to review:</strong> ${escapeHtml(issue)}
              <br><strong>Why:</strong> ${escapeHtml(knowledgeIssueWhy(issue))}
            </li>
          `).join("")}
        </ul>
      </div>
  ...
  function knowledgeReviewFooterHtml(job) {
    ...
    const articleValidation = artifactValidationItems(job.knowledgeAnswer || "");
    ...
    const articleValidationHtml = (!markerRows && articleValidation.length)
      ? `<div class="xa-review-validation-summary"><strong>Article validation items</strong><ol class="xa-review-item-list">${articleValidation.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ol></div>`
      : "";

    const reviewDetails = (markerRows || hasValidation || articleValidationHtml) ? `
  ...
  async function collectCaseEvidence(caseNumber, xsup = "") {
    ...
    const requestedXsup = /^XSUP-\d+$/i.test(String(xsup || "")) ? String(xsup).toUpperCase() : "";
    const hrefLinkedXsup = (
      hrefs.map(h => String(h).match(/\bXSUP-\d+\b/i)?.[0]).find(Boolean) || ""
    ).toUpperCase();
    const textLinkedXsup = (
      `${summaryText}\n${els.map(el => el.innerText || "").join("\n")}`.match(/\bXSUP-\d+\b/i)?.[0] || ""
    ).toUpperCase();
    const linkedXsup = requestedXsup || hrefLinkedXsup || textLinkedXsup;

    const jiraHref =
      hrefs.find(h =>
        /jira-dc\.paloaltonetworks\.com\/browse\/XSUP-\d+/i.test(h) &&
        (!linkedXsup || h.toUpperCase().includes(linkedXsup))
      ) ||
      (linkedXsup ? `https://jira-dc.paloaltonetworks.com/browse/${linkedXsup}` : "");
...
`
  ...
  function buildKnowledgePrompt(job) {
    ...
    const sourceLabel = knowledgeSourceLabel(job);
    const direct = isDirectKcsJob(job);

    const common = `
KNOWLEDGE ENRICHMENT + DRAFT GENERATION

Source Case: ${sourceLabel}
Linked XSUP: ${realXsupForJob(job) || "Not identified / not required"}
SFDC: ${job.caseNumber}
...
${direct
  ? "This is direct KCS mode. Use the supplied case basis and evidence; do not perform or invent a retrospective Support-owned field review."
  : "Do not merely restate the retrospective."}

${direct ? "DIRECT KCS CASE BASIS" : "STARTING RETROSPECTIVE CASE BASIS"}
${job.auditAnswer}
...
AT-A-GLANCE SUMMARY — MANDATORY
- Every generated Knowledge artifact must contain a section named exactly: ## At a Glance
- Place it immediately after the title/draft metadata and before the detailed artifact sections.
- Write 2–3 concise sentences, normally about 35–85 words total.
- It must answer, in plain language:
  1. What problem/topic is this artifact about?
  2. What is the key technical finding/behavior?
  3. What will this artifact help the reader understand, troubleshoot, configure, or resolve?
- Make it understandable to an engineer who has not read the originating XSUP.
- Do not put XSUP/SFDC IDs, source lists, quality-review details, validation-item lists, or internal process commentary in At a Glance.
- Do not introduce any fact in At a Glance that is not supported by the artifact body/evidence.
- Keep the summary reusable across customers.
...
PRELIMINARY INLINE REVIEW MARKERS — REQUIRED DURING GENERATION
Do not wait for the later quality-review prompt to identify every uncertainty.
When the draft contains a material technical detail that is not directly established by an authoritative underlying source in the supplied context, place one of these markers immediately after the affected statement:
- [[SME_REVIEW|reason]] — product behavior, UI path, timing, configuration, or operational detail needs product/SME validation.
- [[ENGINEERING_REVIEW|reason]] — backend, API, architecture, or implementation detail needs Engineering confirmation.
- [[INFERENCE|reason]] — conclusion is derived but not directly established.
- [[SOURCE_CHECK|reason]] — exact command, path, dataset, documentation claim, or factual detail needs a stronger/direct underlying source.
- [[SCOPE_CHECK|reason]] — version, OS, platform, tenant, timing, or applicability scope needs confirmation.
- [[RECOMMENDATION|reason]] — best-practice guidance rather than mandatory product behavior.
- [[CONFIRMED|source cue]] — use sparingly only when a material claim is directly established by an appropriate underlying source.
- [[UNSUPPORTED|reason]] — material claim cannot safely remain as written from the supplied evidence.

Specific high-risk details that MUST be sourced or marked include:
- exact commands or XQL queries,
- exact API routes/payloads,
- exact UI navigation,
- file/log/configuration paths,
- product/agent versions,
- exact timing or propagation windows,
- backend architecture/internal implementation behavior,
- limits/counts/nesting levels,
- remediation behavior and operational guarantees.

Every material item listed later under Validation Items must also have an inline marker beside the specific statement it affects when possible.
Do not hide uncertainty by writing an unsupported detail as a confident fact.
...
## At a Glance
[2–3 sentence plain-language summary of the problem/topic, key finding, and what this article helps the reader do]
...
## At a Glance
[2–3 sentence plain-language summary of the knowledge gap/update, key finding, and what the proposed update helps the reader do]
...
## At a Glance
[2–3 sentence plain-language summary of the documentation topic/gap, key technical finding, and what the proposed guidance helps the reader understand or do]
...
## At a Glance
[2–3 sentence plain-language summary of when this runbook is useful, the key technical pattern, and what the workflow helps TAC determine or resolve]
...
## At a Glance
[2–3 sentence plain-language summary of the issue/limitation, key technical finding, and what the draft helps readers identify or handle]
...
Product: ${productLabel(job)}
Artifact Type: ${label}
Initial ${direct ? "Direct-KCS" : "Retrospective"} Readiness: ${job.artifactReadiness || "DRAFTABLE"}

${direct ? "DIRECT KCS CASE BASIS — AUTHORITATIVE CASE CONTEXT" : "RETROSPECTIVE — AUTHORITATIVE CASE BASIS"}
${job.auditAnswer}

ENRICHED DRAFT TO REVIEW
${draftAnswer}

PRE-QUALITY STRUCTURAL REVIEW NOTES
${Array.isArray(job.knowledgeDraftReviewIssues) && job.knowledgeDraftReviewIssues.length
  ? job.knowledgeDraftReviewIssues.map((issue, i) => `${i + 1}. ${issue}`).join("\n")
  : "None detected."}

If a structural review note is listed above, repair it when the evidence allows. If it cannot be repaired safely, DO NOT discard the draft. Return the best usable draft, set VALIDATED_ARTIFACT_READINESS to NOT READY, and preserve the unresolved issue for human review.
...
- Ensure the final artifact contains ## At a Glance immediately after the title/draft metadata.
- At a Glance must be 2–3 concise, plain-language sentences that state the topic/problem, key technical finding, and what the artifact helps the reader do.
- Ensure At a Glance is understandable without reading the originating XSUP, is reusable across customers, and contains no unsupported new claim, XSUP/SFDC ID, source dump, validation list, or quality/process commentary.
- If the draft's At a Glance is missing, vague, case-specific, misleading, or too long, rewrite it from supported content already present in the supplied case basis/artifact.
...
The supplied case basis or enriched draft may contain internal analytical markers such as:
...
2. If the claim is supported by an underlying source explicitly available in the supplied case basis/draft, rewrite it as a normal sourced statement and cite/identify that underlying source.
...
`.trim();
  ...
  function compactPromptText(value, maxChars = 9000) {
    const text = cleanText(String(value || ""));
    if (text.length <= maxChars) return text;
    const head = Math.floor(maxChars * 0.62);
    const tail = maxChars - head;
    return `${text.slice(0, head)}\n\n[...middle omitted for compact quality retry...]\n\n${text.slice(-tail)}`;
  }

  function isRetryableKnowledgeQualityError(err) {
    const message = cleanText(err?.message || String(err || ""));
    return /(?:422|unprocessable|task rejected|service error|payload|request too large|prompt too large|content too large)/i.test(message);
  }

  function buildCompactKnowledgeQualityPrompt(job, draftAnswer) {
    const type = job.knowledgeArtifactType || knowledgeArtifactType(job);
    const label = knowledgeArtifactLabel(type);
    const sourceLabel = knowledgeSourceLabel(job);

    return `
KNOWLEDGE QUALITY REVIEW — COMPACT RETRY
...
CASE BASIS — COMPACT
${compactPromptText(job.auditAnswer, 8500)}

DRAFT TO REVIEW — KEEP THE COMPLETE REUSABLE CONTENT
${String(draftAnswer || "").trim()}

REVIEW OBJECTIVE
Independently finalize this draft for human review. Do not merely approve the generator.
Check:
1. technical accuracy and evidence support,
2. exact commands/API/UI/path/version/timing/architecture claims,
3. scope/applicability,
4. check → confirm → resolve → verify usefulness,
5. required artifact structure,
6. Source References,
7. customer-specific overfitting,
8. internal metadata/placeholders/raw source markup,
9. consistency between inline markers, Validation Items and readiness.

INLINE MARKERS
Use these exact markers immediately after the affected statement when unresolved:
[[SME_REVIEW|reason]]
[[ENGINEERING_REVIEW|reason]]
[[INFERENCE|reason]]
[[SOURCE_CHECK|reason]]
[[SCOPE_CHECK|reason]]
[[RECOMMENDATION|reason]]
[[CONFIRMED|source cue]]
[[UNSUPPORTED|reason]]

Rules:
- TACO/Case Chat is synthesis, not the underlying factual source.
- Do not remove uncertainty by simply deleting a marker.
- Material SME/Engineering/Inference/Source/Scope items require DRAFTABLE.
- UNSUPPORTED is a blocker: support/rewrite/remove it or return FAIL + NOT READY.
- Remove escaped/raw <ref>, <verification-warning>, internal metadata and placeholders.
- Keep ## At a Glance concise, reusable and evidence-backed.
- Do not invent new commands, APIs, versions, timings, paths, architecture or fixes.
- This remains a human-reviewed draft.
...
1a. Ensure ## At a Glance exists near the top and contains 2–3 concise sentences explaining the topic/problem, key finding, and what the artifact helps the reader do. Repair it using only supported content already available in the supplied material.
...
- At a Glance is present, accurate, reusable, and 2–3 concise sentences,
...
`.trim();
  ...
  function sanitizeKnowledgeSourceMarkup(value) {
    let text = String(value || "");

    // Model/TACO source markup is useful internally but should never render as
    // escaped <ref>/<verification-warning> syntax in a human-facing article.
    text = text.replace(
      /<verification-warning>([\s\S]*?)<\/verification-warning>/gi,
      (_, inner) => `${inner} [[SOURCE_CHECK|The supporting source was flagged for verification; confirm the underlying source before publication.]]`
    );

    text = text.replace(
      /<ref\b([^>]*)>([\s\S]*?)<\/ref>/gi,
      (_, attrs, body) => {
        const url = cleanText(attrs.match(/\burl="([^"]+)"/i)?.[1] || "");
        const title = cleanText(attrs.match(/\btitle="([^"]+)"/i)?.[1] || "Supporting source");
        const quote = cleanText(
          String(body || "")
            .replace(/<q>([\s\S]*?)<\/q>/gi, '"$1"')
            .replace(/<[^>]+>/g, " ")
        );
    ...
    return text
      .replace(/<\/?(?:verification-warning|ref|q)\b[^>]*>/gi, "")
      .replace(/\n{3,}/g, "\n\n")
      .trim();
  }

  function addPreliminaryKnowledgeReviewMarkers(value) {
    ...
    const alreadyMarked = line => reviewMarkerPattern().test(line);

    const add = (line, type, reason) => {
      if (alreadyMarked(line)) return line;
      return `${line} [[${type}|${reason}]]`;
    };
    ...
    return lines.map(rawLine => {
      ...
      if (heading) {
        section = cleanText(heading[1]).toUpperCase();
        return line;
      }

      if (!cleanText(line) || /^\s*#/.test(line) || excludedSections.has(section)) {
        return line;
      }
      ...
      if (
        /\b(?:GET|POST|PUT|PATCH|DELETE)\s+\/[A-Za-z0-9_./{}:-]+/i.test(line) ||
        /\b(?:backend|database pipeline|internal architecture|agent_chunk|calculation job|implementation detail)\b/i.test(line)
      ) {
        checks.push([
          "ENGINEERING_REVIEW",
          "Exact backend/API/implementation behavior should be confirmed against authoritative Engineering or product documentation."
        ]);
      }

      if (
        /\b(?:cytool|xql|pan_dss_raw|CloudIdAgentDebug|CloudIdAgentConfig)\b/i.test(line) ||
        /\\Program Files(?: \(x86\))?\\/i.test(line) ||
        /Endpoints\s*(?:→|->|>|\/)\s*All Endpoints/i.test(line) ||
        /\bEndpoint Control\b/i.test(line)
      ) {
        checks.push([
          "SOURCE_CHECK",
          "Exact command, dataset, path, or UI detail should be verified against an authoritative underlying source."
        ]);
      }

      if (
        /\b\d+(?:\.\d+)?\s*(?:-|to)?\s*\d*(?:\.\d+)?\s*(?:minutes?|hours?|days?)\b/i.test(line) ||
        /\b\d+(?:\.\d+)?[- ]hour\b/i.test(line) ||
        /\b(?:agent\s+)?versions?\s+[0-9x.,\s/-]+/i.test(line) ||
        /\bup to\s+\d+\s+(?:domain\s+)?(?:nesting\s+)?levels?\b/i.test(line)
      ) {
        checks.push([
          "SCOPE_CHECK",
          "Exact timing/version/applicability scope should be confirmed before publication."
        ]);
      }

      if (
        /\b(?:CPU starvation|thread contention|third-party antivirus|recursive group membership)\b/i.test(line)
      ) {
        checks.push([
          "SOURCE_CHECK",
          "This additional troubleshooting claim needs a direct supporting source before it can be treated as reusable product guidance."
        ]);
      }

      if (/\b(?:recommended|best practice|prefer|long-term solution)\b/i.test(line)) {
        checks.push([
          "RECOMMENDATION",
          "This is guidance/recommendation rather than a mandatory product behavior."
        ]);
      }

      // Keep fallback annotation readable: at most two materially distinct
      // markers per Markdown line. The independent quality reviewer may remove,
      // confirm, narrow, or replace these based on stronger evidence.
      const seen = new Set();
      for (const [type, reason] of checks) {
        if (seen.has(type)) continue;
        seen.add(type);
        line = add(line, type, reason);
  ...
  function resolveRawProvenanceForHumanReview(text) {
    return addPreliminaryKnowledgeReviewMarkers(
      sanitizeKnowledgeSourceMarkup(
        stripInternalKnowledgeMetadata(text)
          .replace(
            /\[inference\]/gi,
  ...
  function deterministicKnowledgeQualityChecks(artifact, job, requestedReadiness = "") {
    ...
    if (/<\/?(?:ref|verification-warning|q)\b/i.test(text)) issues.push("raw source/provenance markup is visible");

    const fences = (text.match(/```/g) || []).length;
    if (fences % 2 !== 0) issues.push("Markdown code fence is not balanced");

    const requiredHeadingSets = {
      KCS_DRAFT: ["At a Glance", "Symptoms / Error", "Cause", "How to Check", "How to Confirm", "Resolution / Fix", "Source References"],
      KCS_UPDATE: ["At a Glance", "Existing Knowledge Reference", "Gap Identified", "Proposed Additions / Changes", "Source References"],
      DOC_UPDATE: ["At a Glance", "Target Documentation", "Documentation Gap", "Proposed Documentation Text", "Source References"],
      RUNBOOK: ["At a Glance", "Trigger / When to Use", "Objective", "Investigation Workflow", "Decision Points", "Source References"],
      KNOWN_ISSUE: ["At a Glance", "Issue", "Symptoms", "Cause / Limitation", "Proposed Release Note / Known Issue Text", "Source References"]
    };
    ...
    const atAGlance = extractKnowledgeSection(text, ["At a Glance"]);
    if (atAGlance) {
      const summaryPlain = cleanText(
        atAGlance
          .replace(reviewMarkerPattern(), "")
          .replace(/[`*_>#-]/g, " ")
      );
      const summarySentences = (summaryPlain.match(/[^.!?]+[.!?]+(?:\s|$)/g) || [])
        .map(s => cleanText(s))
        .filter(Boolean);
      const summaryWords = summaryPlain.split(/\s+/).filter(Boolean).length;

      if (summarySentences.length < 2 || summarySentences.length > 3) {
        issues.push("At a Glance should contain 2–3 concise sentences");
      }
      if (summaryWords < 20) {
        issues.push("At a Glance is too brief to explain the topic, key finding, and reader value");
      }
      if (summaryWords > 110) {
        issues.push("At a Glance is too long; keep it concise and scannable");
      }
      if (/\bXSUP-\d+\b/i.test(summaryPlain) || (job.caseNumber && new RegExp(`\\b${String(job.caseNumber).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`).test(summaryPlain))) {
        issues.push("At a Glance contains originating XSUP/SFDC case identifiers");
      }
      if (/quality review|quality gate|validation items?|case chat|TACO analysis/i.test(summaryPlain)) {
        issues.push("At a Glance contains internal generation/review-process commentary");
      }
    }

    const generatedFrom = extractField(text, "Generated From");
    const expectedGeneratedFrom = knowledgeSourceLabel(job);
    if (generatedFrom && !generatedFrom.toUpperCase().includes(expectedGeneratedFrom.toUpperCase())) {
      issues.push(`artifact targets ${generatedFrom}, not ${expectedGeneratedFrom}`);
    }
    ...
    const blocking = issues.some(issue =>
      /incomplete|internal reuse metadata|unresolved internal placeholder|raw internal provenance|raw source\/provenance markup|unresolved editorial placeholder|code fence|missing required section|artifact targets|Source References|unsupported review marker/i.test(issue)
    );
  ...
  function buildKnowledgeReuseMeta(job) {
    ...
    const fingerprint = stableHashText(JSON.stringify({
      ...
      workflowMode: job.workflowMode || "retrospective",
      source: knowledgeSourceLabel(job),
      xsup: realXsupForJob(job),
      caseNumber: job.caseNumber,
      investigationId: job.investigationId,
  ...
  function validateReusableKnowledgeAnswer(answer, job, reuseType = "knowledge") {
    ...
    const a = reuseType === "knowledge_draft"
      ? resolveRawProvenanceForHumanReview(raw)
      : raw;

    if (a.length < 160) return {valid:false, reason:"artifact is incomplete", artifact:a};
    ...
    if (expected && !expected.test(a)) {
      return {valid:false, reason:`result does not match ${knowledgeArtifactLabel(type)} structure`, artifact:a};
    }

    const generatedFrom = extractField(a, "Generated From");
    const expectedGeneratedFrom = knowledgeSourceLabel(job);
    if (generatedFrom && !generatedFrom.toUpperCase().includes(expectedGeneratedFrom.toUpperCase())) {
      return {valid:false, reason:`artifact targets ${generatedFrom}, not ${expectedGeneratedFrom}`, artifact:a};
    }

    // Internal reuse metadata is tool plumbing, not a reviewer-facing content problem.
    // It is stripped before draft validation and must never block generation.
    return {valid:true, artifact:a};
  }
  ...
  function resetDerivedOutputsForProductChange(job) {
    ...
    job.knowledgeDraftReviewIssues = [];
    job.knowledgeQualityStatus = "";
    job.knowledgeQualityIssues = [];
  ...
  function renderJobList() {
    ...
    const jobs = [...state.jobs.values()];
    if (!jobs.length) {
      list.innerHTML = `<div class="xa-job-empty">Enter XSUP IDs for retrospective review, or XSUP/SFDC case numbers for direct KCS generation.</div>`;
      return;
    }
  ...
  function renderExecutionPipeline(job) {
    ...
    const stages = [
      ...
      ["audit", isDirectKcsJob(job) ? "Retrospective field review" : "Retrospective audit",
        isDirectKcsJob(job)
          ? "SKIPPED · Direct KCS mode"
        ...
        isDirectKcsJob(job)
          ? "Direct KCS mode intentionally skips the Support-owned retrospective field review."
          : "Shows whether the audit was reused from existing Case Chat history or generated newly."],
      ["knowledge", isDirectKcsJob(job) ? "KCS generation & quality" : "Knowledge artifact", knowledgeStageDetail(job), "Shows whether the knowledge artifact was reused or newly generated."],
    ...
    box.innerHTML = `
      <div class="xa-reuse-head">
        <div>
          ...
          <span>${direct
            ? "Direct KCS mode reuses current TACO/evidence and existing compatible KCS results when safe."
            : "Shows exactly which existing Case Chat was reused, or whether a new result had to be generated."}</span>
        </div>
        <div class="xa-reuse-actions">
          <button id="xa-reanalyse-all" ${busy ? "disabled" : ""} data-tooltip="${direct
            ? "Force a fresh TACO analysis, then generate a fresh KCS. The retrospective field review remains skipped."
            : "Force a fresh TACO analysis, then create a new retrospective audit and new knowledge artifact. Use only when you intentionally want to ignore all reusable results."}">Re-analyze All</button>
        </div>
      </div>
      <div class="xa-reuse-grid">
        ...
        ${card({
          label: direct ? "Direct KCS" : "Knowledge Artifact",
          ...
          actionLabel: direct ? "Regenerate KCS" : knowledgeActionLabel,
          actionDisabled: knowledgeActionDisabled,
          actionTooltip: direct
            ? "Create a new KCS from the current TACO analysis and original evidence. The retrospective field review is not run."
            : "Create a new knowledge artifact from the current completed audit using the latest enrichment and quality-review workflow. TACO and the audit are not re-run."
        })}
      </div>
    `;
  ...
  function renderDecisionSummary(job) {
    ...
    if (isDirectKcsJob(job)) {
      const readiness = job.validatedArtifactReadiness || job.artifactReadiness || "DRAFTABLE";
      const reviewCount = knowledgeReviewCount(job);
      box.innerHTML = `
        <div class="xa-decision-group">
          <div class="xa-decision-group-title">Direct KCS Generation</div>
          <div class="xa-decision-table-wrap">
            <table class="xa-decision-table">
              ...
              <tbody>
                <tr>
                  <td><strong>Retrospective field review</strong></td>
                  <td>${semanticChipHtml("NOT APPLICABLE", "eligibility")}</td>
                  <td class="xa-table-explain">Skipped intentionally. Direct KCS mode does not review Resolution, RCA, Fix Type, or Labels.</td>
                </tr>
                <tr>
                  <td><strong>KCS draft</strong></td>
                  <td>${semanticChipHtml(readiness, "readiness")}</td>
                  <td class="xa-table-explain">${escapeHtml(
                    job.knowledgeStatus === "completed"
                      ? `${reviewCount ? `${reviewCount} review item${reviewCount === 1 ? "" : "s"} remain. ` : ""}Human review is required before publication.`
                      : "KCS generation and quality review are in progress or queued."
                  )}</td>
                </tr>
              </tbody>
            </table>
  ...
  function renderKnowledgeArtifact(job) {
    ...
    box.innerHTML = `
      <div class="xa-knowledge-card">
        ...
        ${knowledgeBlockingReviewHtml(job)}
        ${job.knowledgeError && normalizeDecision(job.validatedArtifactReadiness) !== "NOT READY" ? `<div class="xa-knowledge-error">${escapeHtml(job.knowledgeError)}</div>` : ""}
  ...
  function renderSelectedJob() {
    ...
    if (title) {
      title.textContent = `${jobDisplayId(job)}${job.productKey ? ` · ${productLabel(job)}` : ""}${isDirectKcsJob(job) ? " · Direct KCS" : ""}`;
    }

    showReport(job.auditAnswer || (job.error ? `## ${isDirectKcsJob(job) ? "KCS Preparation" : "Audit"} Error\n\n**${job.error}**` : ""));
    ...
    const direct = isDirectKcsJob(job);
    for (const id of [
      "xsup-auditor-audit-report-title",
      "xsup-auditor-output",
      "xsup-auditor-audit-report-actions",
      "xsup-auditor-review-comment-title",
      "xsup-auditor-xsup-comment",
      "xsup-auditor-review-comment-actions"
    ]) {
      const el = document.getElementById(id);
      if (el) el.style.display = direct ? "none" : "";
    }

    const retryBtn = document.getElementById("xsup-auditor-retry-chat");
    if (retryBtn) {
      retryBtn.style.display = direct ? "none" : "";
      retryBtn.disabled = direct || state.running || !job.lastPrompt || !job.caseNumber || !job.investigationId;
    }
  ...
  function isDirectKcsJob(job) {
    return job?.workflowMode === "direct_kcs";
  }

  function realXsupForJob(job) {
    const candidates = [job?.linkedXsup, job?.xsup, job?.sourceInput];
    for (const value of candidates) {
      const match = String(value || "").toUpperCase().match(/^XSUP-\d+$/);
  ...
  function jobDisplayId(job) {
    const xsup = realXsupForJob(job);
    if (xsup && job?.caseNumber) return `${xsup} · SFDC ${job.caseNumber}`;
    if (xsup) return xsup;
    if (job?.caseNumber) return `SFDC ${job.caseNumber}`;
    return String(job?.sourceInput || job?.xsup || "Case");
  }

  function knowledgeSourceLabel(job) {
    return realXsupForJob(job) || (job?.caseNumber ? `SFDC ${job.caseNumber}` : String(job?.sourceInput || job?.xsup || "Case"));
  }

  function createJob(xsup, options = {}) {
    const requestedXsup = /^XSUP-\d+$/i.test(String(options.linkedXsup || xsup || ""))
      ? String(options.linkedXsup || xsup).toUpperCase()
      : "";
    const caseNumber = String(options.caseNumber || "");
    return {
      xsup,
      workflowMode: options.workflowMode || "retrospective",
      sourceInput: options.sourceInput || xsup,
      linkedXsup: requestedXsup,
      status: "queued",
      stageLabel: "Queued",
      steps: {
        resolve: "Waiting",
      ...
      references: [],
      targetLinks: {
        jira: requestedXsup ? `https://jira-dc.paloaltonetworks.com/browse/${requestedXsup}` : "",
        sfdc: "",
        tacopilot: caseNumber ? `${location.origin}/taco/case/${caseNumber}` : ""
      },
      lastPrompt: "",
      verdict: "",
  ...
  function parseDirectKcsInputs(raw) {
    const matches = String(raw || "").toUpperCase().match(/\bXSUP-\d+\b|\b\d{8}\b/g) || [];
    const seen = new Set();
    const parsed = [];
    for (const token of matches) {
      const type = /^XSUP-\d+$/.test(token) ? "xsup" : "sfdc";
      const key = `${type}:${token}`;
      if (seen.has(key)) continue;
      seen.add(key);
      parsed.push({ type, value: token });
    }
    return parsed;
  }

  function addJobsFromInput(workflowMode = "retrospective") {
    const input = document.getElementById("xsup-auditor-input");
    const raw = input?.value || "";
    const direct = workflowMode === "direct_kcs";
    const items = direct
      ? parseDirectKcsInputs(raw)
      : parseXSUPs(raw).map(value => ({ type: "xsup", value }));

    if (!items.length) {
      alert(
        direct
          ? "Enter one or more XSUP IDs or 8-digit SFDC case numbers, for example:\\nXSUP-72446\\n04005807"
          : "Enter one or more XSUP IDs, for example:\\nXSUP-72446\\nXSUP-81234"
      );
      return [];
    }
    ...
    for (const item of items) {
      const isSfdc = item.type === "sfdc";
      const key = isSfdc ? `SFDC:${item.value}` : item.value;
      ...
      const job = createJob(key, {
        ...
        caseNumber: isSfdc ? item.value : "",
        linkedXsup: isSfdc ? "" : item.value
      });
  ...
  function buildDirectKcsCaseBasis(job, selected) {
    const report = job.report || {};
    const structured = job.evidence?.structured_fields || {};
    const conclusion =
      report?.verified_conclusion ||
      report?.result?.rca ||
      report?.final_report ||
    ...
    const hypotheses = Array.isArray(report?.hypotheses)
      ? report.hypotheses
          .map((h, i) => `${i + 1}. ${cleanText(h?.statement || h?.conclusion || h?.description || "")}`)
          .filter(x => !/\.\s*$/.test(x) ? cleanText(x.replace(/^\d+\.\s*/, "")) : true)
          .join("\n")
      : "";

    const fieldLines = Object.entries(structured)
      .filter(([, value]) => value != null && cleanText(String(value)))
      .slice(0, 30)
      .map(([key, value]) => `- ${key}: ${cleanText(String(value)).slice(0, 500)}`)
      .join("\n");

    return `
DIRECT KCS CASE BASIS — NOT A RETROSPECTIVE FIELD REVIEW

Source case: ${knowledgeSourceLabel(job)}
SFDC Case: ${job.caseNumber}
Linked XSUP: ${realXsupForJob(job) || "Not identified / not required"}
Product: ${productLabel(job)}

TACO SYNTHESIZED TECHNICAL CONCLUSION
${cleanText(String(conclusion)) || "Not available"}

TACO HYPOTHESES / TECHNICAL CONTEXT
${hypotheses || "None separately listed."}

STRUCTURED CASE FIELDS
${fieldLines || "No structured fields were extracted."}

FOCUSED ORIGINAL JIRA EVIDENCE
${formatRecords("JIRA", selected?.jira || [])}

FOCUSED SFDC INTERNAL EVIDENCE
${formatRecords("SFDC INTERNAL", selected?.internal || [])}

FOCUSED TAC PUBLIC EVIDENCE
${formatRecords("TAC PUBLIC", selected?.tac_public || [])}

FOCUSED CUSTOMER PUBLIC EVIDENCE
${formatRecords("CUSTOMER PUBLIC", selected?.customer_public || [])}

IMPORTANT
- This basis is provided only to generate a reusable KCS draft.
- It is not a retrospective verdict and does not review Resolution, RCA, Fix Type, or Labels.
- TACO is synthesized analysis; original Jira/SFDC and underlying documentation/Engineering sources remain the evidence boundary for factual claims.
`.trim();
  }

  // ---------------------------------------------------------------------------
  // Per-case worker
  // retrospective mode: resolve -> TACO/evidence -> audit -> optional knowledge
  // direct KCS mode: resolve -> TACO/evidence -> KCS quality pipeline
  // ---------------------------------------------------------------------------
  ...
  async function processJob(job) {
    ...
    try {
      if (!job.caseNumber) {
        ...
        const candidates = await resolveXSUPCandidates(realXsupForJob(job) || job.sourceInput || job.xsup);
        ...
        const resolvedXsup = realXsupForJob(job);
        job.targetLinks = {
          jira: resolvedXsup ? `https://jira-dc.paloaltonetworks.com/browse/${resolvedXsup}` : "",
      ...
      job.evidence = await collectCaseEvidence(job.caseNumber, realXsupForJob(job));
      if (!job.linkedXsup && job.evidence?.linked_xsup) {
        job.linkedXsup = String(job.evidence.linked_xsup).toUpperCase();
      }
      ...
      if (!job.productKey || job.productSelectionSource !== "manual") {
        ...
        if (requireConfirmation && job.productSelectionSource !== "manual") {
          ...
          renderDashboard();
          if (job.xsup === state.selectedXsup) renderSelectedJob();
          showToast(`${jobDisplayId(job)}: confirm product before ${isDirectKcsJob(job) ? "KCS generation" : "the retrospective"} starts`, "ok");
          setTimeout(showNextProductChooser, 80);
          return;
        }
      }
      ...
      if (isDirectKcsJob(job)) {
        ...
        job.auditReportedProduct = productLabel(job);
        job.auditReuseStatus = "not_required";
        job.auditReuseReason = "Direct KCS mode intentionally skips the retrospective Support-owned field review.";
        job.auditFollowupId = null;
        ...
        job.artifactTypeFromAudit = "KCS Draft";
        job.knowledgeDecisionExplanation =
          "Direct KCS mode was requested. The case is sent directly through KCS generation, independent quality review, deterministic checks, and human review without running the retrospective field-review prompt.";
        job.autoGenerateKnowledgeDecision = "YES";
        job.knowledgeArtifactType = "KCS_DRAFT";
        job.auditAnswer = buildDirectKcsCaseBasis(job, selected);
        job.auditFingerprint = "";
        job.lastPrompt = "";
        ...
        setJobStep(job, "audit", "Skipped · Direct KCS mode", "KCS generation", {
          activity: "Case context ready · retrospective field review skipped · KCS queued"
        });

        if (job.xsup === state.selectedXsup) renderSelectedJob();
        queueKnowledgeArtifact(job);
        showToast(`✓ ${jobDisplayId(job)} case context ready · KCS generation queued`, "ok");
        return;
      }
    ...
    try {
      ...
      if (finalReuse.reused) {
        ...
        if (draftReuse.reused) {
          job.knowledgeDraftAnswer = resolveRawProvenanceForHumanReview(draftReuse.answer);
          job.knowledgeDraftReviewIssues = [];
          ...
          const reviewableDraft = resolveRawProvenanceForHumanReview(draftRaw);
          job.knowledgeDraftAnswer = reviewableDraft;

          if (reviewableDraft.length < 160) {
            throw new Error("Knowledge enrichment did not return a usable draft.");
          }
          ...
          job.knowledgeDraftReviewIssues = draftValidation.valid
            ? []
            : [draftValidation.reason || "Enriched draft needs structural review."];

          if (job.knowledgeDraftReviewIssues.length) {
            update(`Draft generated with ${job.knowledgeDraftReviewIssues.length} structural review item · continuing to independent quality review...`);
          }
        ...
        try {
          ...
          qualityFollowupId = directQualityId || await waitForFollowupId(
            job.caseNumber,
            job.investigationId,
            qualityTaskId,
            qualityPrompt,
          ...
          qualityRaw = await waitForFollowup(
            job.caseNumber,
            qualityFollowupId,
            update
          );
        } catch (qualityErr) {
          if (!isRetryableKnowledgeQualityError(qualityErr)) throw qualityErr;

          update("Quality review request was rejected · retrying once with compact quality context...");

          const compactBase = buildCompactKnowledgeQualityPrompt(
            job,
            job.knowledgeDraftAnswer
          );
          const compactPrompt = appendReuseMarker(compactBase, meta);

          const submitCompact = await postFollowup(
            job.caseNumber,
            job.investigationId,
            compactPrompt
          );
          const compactTaskId = submitCompact?.task_id;
          if (!compactTaskId) {
            throw new Error(`Compact knowledge quality retry did not return task_id after initial error: ${qualityErr?.message || qualityErr}`);
          }

          const directCompactId = extractFollowupId(submitCompact);
          qualityFollowupId = directCompactId || await waitForFollowupId(
            job.caseNumber,
            job.investigationId,
            compactTaskId,
            compactPrompt,
          ...
          job.knowledgeFollowupId = qualityFollowupId;
          update(`Compact quality review Case Chat #${qualityFollowupId}`);
        ...
        if (!parsed.valid) {
          ...
          job.knowledgeQualityValidationItems = parsed.validationItems;
          job.knowledgeQualityIssues = Array.isArray(parsed.issues)
            ? parsed.issues.map(cleanText).filter(Boolean)
            : [];
        }
      ...
      showToast(
        `${finalKnowledgeReadiness === "READY" ? "✓" : "⚠"} ${jobDisplayId(job)} ${knowledgeArtifactLabel(job.knowledgeArtifactType)} ${job.knowledgeReuseStatus === "reused" ? "reused" : "generated"} · ${finalKnowledgeReadiness || "REVIEW"}`,
      ...
      if (err?.name === "AbortError" || state.stopped) {
        ...
        job.knowledgeQualityStatus = "QUALITY_REVIEW_ERROR";
        job.validatedArtifactReadiness = "NOT READY";
        job.knowledgeQualitySummary = job.knowledgeError;
        job.knowledgeQualityIssues = [job.knowledgeError];
        ...
        job.references = extractReferences(job.auditAnswer, job.knowledgeAnswer);
        await downloadKnowledgeArtifact(job, { auto: true });
        showToast(`⚠ ${jobDisplayId(job)} knowledge draft preserved · NOT READY`, "");
      } else {
        ...
        showToast(`⚠ ${jobDisplayId(job)} knowledge generation failed before a usable draft was produced`, "error");
      }
      job.knowledgeEndedAt = Date.now();
    } finally {
  ...
  function runAudit() {
    ...
    const added = addJobsFromInput("retrospective");
  ...
  function runDirectKcs() {
    if (state.stopped && state.activeCount > 0) {
      setStatus("Finishing Stop All; wait a moment before starting a new batch.", "error");
      return;
    }

    const added = addJobsFromInput("direct_kcs");
    if (!added.length) return;
...
.xa-at-a-glance{margin:12px 0 18px;padding:10px 12px;border:1px solid #bfdbfe;border-left:4px solid #60a5fa;border-radius:9px;background:#f8fbff}
.xa-at-a-glance-title{font-size:12px;font-weight:800;color:#1e40af;margin-bottom:4px}
.xa-at-a-glance-body{font-size:13px;color:#334155;line-height:1.5}
.xa-at-a-glance-body p{margin:0}
.xa-at-a-glance-body p+p{margin-top:5px}
.xa-publication-blocker{margin:12px 0 16px;padding:11px 13px;border:1px solid #fca5a5;border-left:5px solid #dc2626;border-radius:9px;background:#fef2f2;color:#7f1d1d}
.xa-publication-blocker-title{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px}
.xa-publication-blocker-title strong{font-size:12px;color:#991b1b}
.xa-publication-blocker p{margin:5px 0 8px;font-size:12px;color:#7f1d1d}
.xa-publication-blocker ul{margin:6px 0 0;padding-left:20px}
.xa-publication-blocker li{margin:7px 0;font-size:12px;line-height:1.45;color:#7f1d1d}
...
${bodyHtml}
  ...
  function knowledgeArticleHtmlWithStatus(job) {
    ...
    const blocker = knowledgeBlockingReviewHtml(job);
    if (blocker) {
      const titleEnd = articleHtml.indexOf("</div>");
      articleHtml = titleEnd >= 0
        ? articleHtml.slice(0, titleEnd + 6) + blocker + articleHtml.slice(titleEnd + 6)
        : blocker + articleHtml;
    }
    return articleHtml;
  }
    ...
    return `
      <details class="xa-meta-details">
        ...
        <div class="xa-meta-details-body">
          <table class="xa-detail-table">
            <tbody>
              <tr><th>Source</th><td>${escapeHtml(knowledgeSourceLabel(job))}</td></tr>
              <tr><th>Linked XSUP</th><td>${escapeHtml(realXsupForJob(job) || "—")}</td></tr>
              <tr><th>SFDC</th><td>${escapeHtml(job.caseNumber || "—")}</td></tr>
              <tr><th>Workflow</th><td>${escapeHtml(isDirectKcsJob(job) ? "Direct KCS" : "Retrospective → Knowledge")}</td></tr>
              <tr><th>Product</th><td>${escapeHtml(productLabel(job))}</td></tr>
              <tr><th>Knowledge action</th><td>${escapeHtml(job.knowledgeAction || "—")}</td></tr>
    ...
    modal.innerHTML = `
      <div class="xa-modal xa-help-modal">
        ...
        <div class="xa-help-body">
          ...
          <h3>Two entry workflows</h3>
          <p><b>Run XSUP Retrospective</b> is the existing field-review workflow. It accepts XSUP IDs, reviews the product-specific Support-owned fields, and then generates Knowledge when applicable.</p>
          <p><b>Generate KCS</b> accepts either an XSUP ID or an 8-digit SFDC case number. It resolves/reuses TACO, collects original Jira/SFDC evidence, then generates a KCS Draft and runs the same independent quality review, deterministic checks, optional one-time repair, and human-review boundary. It intentionally skips Resolution/RCA/Fix Type/Label retrospective review.</p>
          ...
          <p>The final gate checks usefulness, completeness, actionability, generalization, evidence support, source relevance, consistency, readability, discoverability, audience fit and verification. Material validation items downgrade the final artifact to <b>DRAFTABLE</b>. A usable draft with a material blocker is preserved as <b>NOT READY</b> with a visible review-required explanation; only a true generation failure with no usable draft is treated as failed.</p>
  ...
  function createUI() {
    ...
    panel.innerHTML = `
      ...
      <div class="xa-input-row">
        <textarea id="xsup-auditor-input" placeholder="Retrospective: XSUP IDs only&#10;Direct KCS: XSUP or SFDC case&#10;XSUP-72446&#10;04005807"></textarea>
        <button id="xsup-auditor-run">Run XSUP Retrospective</button>
        <button id="xsup-auditor-generate-kcs" title="Generate a KCS directly from an XSUP or SFDC case. Skips the retrospective field-review prompt but keeps TACO freshness, original evidence, quality review, deterministic checks and human review.">Generate KCS</button>
        <button id="xsup-auditor-stop" disabled>Stop All</button>
      </div>
      <div class="xa-input-help"><b>Run XSUP Retrospective:</b> enter XSUP IDs. <b>Generate KCS:</b> enter XSUP IDs or 8-digit SFDC case numbers. IDs may be separated by spaces, commas or new lines. Direct KCS skips Resolution/RCA/Fix Type/Label review and goes straight through the full KCS quality pipeline.</div>
      ...
      <div class="xa-workspace">
        <aside class="xa-sidebar">
          <div class="xa-side-head"><span>Case Queue</span><span>2 workers</span></div>
          ...
          <div id="xsup-auditor-job-list" class="xa-job-list">
            <div class="xa-job-empty">Enter XSUP IDs for retrospective review, or XSUP/SFDC case numbers for direct KCS generation.</div>
          </div>
        ...
        <main class="xa-detail">
          ...
          <div id="xsup-auditor-detail-empty" class="xa-detail-empty" style="display:none">
            Select a case from the queue to view its progress, audit/KCS status and evidence.
          </div>
          ...
          <div id="xsup-auditor-detail-content" style="display:none">
            ...
            <div id="xsup-auditor-audit-report-title" class="xa-section-title">Audit Report</div>
            ...
            <div id="xsup-auditor-audit-report-actions" class="xa-actions">
            ...
            <div id="xsup-auditor-review-comment-title" class="xa-section-title">Review Paste Comment</div>
            <textarea id="xsup-auditor-xsup-comment" placeholder="Review paste comment will appear here..."></textarea>
            <div id="xsup-auditor-review-comment-actions" class="xa-actions">
    ...
    document.getElementById("xsup-auditor-generate-kcs").onclick = runDirectKcs;
